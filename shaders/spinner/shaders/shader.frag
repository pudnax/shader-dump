...
void set_camera(out vec3 pos, out vec3 ray, in vec3 origin, in vec2 rotation, in float dist, in float zoom, in vec2 uv) {
...
vec2 noise(in vec3 x) {
    ...
    float o = textureLod(float_texture1, (uv + 0.5) / 256.0, 0.0).r;
    float s = textureLod(float_texture2, (uv + 0.5) / 256.0, 0.0).r;
    float t =  o * s;
    float rd1 = texture(float_texture1, in_uv).r;
    float rd2 = texture(float_texture2, in_uv).r;
    return vec2(rd1, rd2);
}
...
//    float a = abs(fract(atan(pos.x, pos.z) / slice /* + time * 2.5 */) - 0.5) * slice;
        /*  */
        /*     pos.xz = length(pos.xz) * vec2(sin(a), cos(a)); */
        /*     pos.y = abs(pos.y); */
        /*  */
        /*     float d = dot(pos, normalize(vec3(1.))) - 1.; */
...
void main() {
    ...
    float zoom = 2.4;
    vec3 origin = .009 * vec3(noise(vec3(2.0 * time, 0., 0.)).xy, 0.);
    ...
    color *= mix(vec3(1), mix(vec3(1, .5, 0), vec3(0, .5, 1), filmNoise.x),
               .1 * pow(filmNoise.y, 1.0));
    ...
    color *= vec3(1.0) - .5 * vec3(.3, .5, .7) *
                           pow(1.0 - smoothstep(.0, .1, scratches), 2.0);
