...
vec3 scene(vec2 uv, float t) {
    uv *= 1.7;
    uv *= rot((sin(t * 1.0) * 0.5 + 0.5) * 3. * PI + PI/2);
    uv = abs(uv);
    float sd = max(uv.x - 0.5, uv.y - 1.5);
    return vec3(AAstep(0.03, sd));
}
...
#define BLUR 30
        for (int i = 0; i < BLUR; i++) {
            ...
            rnd = PRand(uint(length(uv)*10  + i)).x;
            float time = pc.time + rnd / 1;// / 60.0;
            float subsample = prand.y;
            time = pc.time + ((float(i) + subsample) / float(BLUR) - 0.5) *
                                 pc.time_delta;
            res += scene(uv, time);
        }
        res /= float(BLUR);
