...
vec3 standardObserver1931[] =
    vec3[](vec3(0.001368, 0.000039, 0.006450),  // 380 nm
...
vec3 process_hit(inout float dist, vec3 r, vec3 p, inout float side, float ior) {
    ...
    vec3 n = side * normalize(p + r * dist -
                              vec3(map(p - off.xyy), map(p - off.yxy),
                                   map(p - off.yyx)));
    /* n = side * normalize(dist - */
    /*                           vec3(map(p - off.xyy), map(p - off.yxy), */
    /*                                map(p - off.yyx))); */
    /* n = norm(p + r * dist); */
    n = norm(p);
...
void trace_lazer(vec3 ro, vec3 rd, float ior) {
    vec3 p = ro;
    points[0] = p;
    pid = 1;
    float side = 1.;
...
vec3 XYZtosRGB(vec3 XYZ) {
    ...
    mat3 m = mat3(3.2404542, -1.5371385, -0.4985314, -0.9692660, 1.8760108,
...
void main() {
    ...
    float bayer_jitter = get_bayer_from_coord(in_uv * pc.resolution);

    vec3 lazer_start = vec3(18., 0., 0.);
    vec3 lazer_dir = normalize(vec3(-1., sin(time) * 0.07, 0));

    float ior = rand() - 0.5;
    ior = bayer_jitter - 0.5;
    float id = ior * 2.;
    vec3 diff = 1.3 - vec3(1. + id, 0.45 + abs(id), 1. - id);

    /* float wv = mix(420., 750., rand()); */
    /* wv = mix(420., 750., bayer_jitter); */
    /*  id = DispersionLaw(wv);  */
    /* float blackb = 1e-13 * BlackBody(6000., wv); */
    /* vec3 scol = blackb * WavelengthToXYZLinear(wv); */
    /* vec3 att = scol; */
    ...
    /* diff = 0.0 + normalize(vec3(1. + scol.x, 0.8 + abs(scol.y), 1. + scol.z)); */
    /* diff = scol; */

    trace_lazer(lazer_start, lazer_dir, ior);
    ...
    col = mix(vec3(amb), lazer * 1., vec3(0.9));

    /* col = smoothstep(0.01, 0.9, col); */
    /* col = pow(col, vec3(1.4544)); */
