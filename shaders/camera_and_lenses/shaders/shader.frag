...
void main() {
	...
	vec2 iResolution = pc.resolution;
	vec2 iMouse = (pc.mouse) * 0.3 * iResolution;
    ...
    vec3 focusrd = normalize(
        vec3(-1., 0.6 * (iMouse.xy * 2. - iResolution.xy) / iResolution.y));
    float focusingDistance =
        iMouse.x > 0. ? abs((ro + focusrd * render(ro, focusrd).w).x - ro.x) : 2.;
    ...
    for (float x = 0.0; x <= 6.0; x += 1.) {
        for (float y = 0.0; y <= 6.0; y += 1.) {
            ...
            vec3 rd =
                normalize(vec3(-focusingDistance,
                               (uv + aa) * focusingDistance * 0.6 + offset));
    ...
    color *= (1.0 / 36.);
            ...
            /* vec2 offset = random_in_unit_disk(seed) * aperture; */
    /* vec3 rd = normalize( */
        /* vec3(-focusingDistance, (uv) * focusingDistance * 0.6 + offset)); */
    /* color = render(ro, rd).rgb; */

    out_color = vec4(pow(color.rgb, vec3(1.45454)), 1.0);
}
