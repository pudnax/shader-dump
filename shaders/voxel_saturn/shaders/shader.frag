...
float noise(in vec3 p) {
    ...
    return mix(
        mix(random(id + vec3(0.0)), random(id + vec3(1.0, 0.0, 0.0)), u.x),
        mix(random(id + vec3(0.0, 1.0, 1.0)), random(id + vec3(1.0, 1.0, 1.0)),
            u.x),
        u.y);
}
...
float noise_fbm(vec3 p) {
    float h = fbm(0.09 * pc.time + p +
                  fbm(0.065 * pc.time + 2.0 * p - 5.0 * fbm(4.0 * p)));
    return h;
}
...
float map(vec3 p) {
    ...
    sphere = max(sphere, abs(fract(ps.x + pc.time * 0.3)) - .8);
...
vec3 norm(vec3 p) {
    ...
    return normalize(map(p) - vec3(map(k[0]), map(k[1]), map(k[1])));
}

vec3 hsv(float h, float s, float v) {
    return mix(vec3(1.0),
               clamp((abs(fract(h + vec3(3.0, 2.0, 1.0) / 3.0) * 6.0 - 3.0) -
                      1.0),
                     0.0, 1.0),
               s) *
           v;
