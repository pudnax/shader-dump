...
float scene(vec3 p) {
    ...
    /*     float bending_plane = */
    /*         max((pm.y + 0.045), (length(pm + vec3(0., 0., -0.2)) - .35)); */
    /*     tongue.x = smin(tongue.x, bending_plane, 0.057); */
    /*     /1* tongue += noiseier;// * pow(umuer.y, 3); *1/ */
...

    {
        ...
        vec3 dripping =
            sdCappedCone(pi, vec3(0.), vec3(0.0, 0.14, 0.0), 0.27 , 0.20) + 0.024*(1-amplifier);
        dripping.z += -0.113;
        float waveFactor = (d > 1. ? 1. : .5);
        float wave = sin(ang * 20. * waveFactor) * .067 +
                     sin(ang * 45. * waveFactor + .7) * 0.038 +
