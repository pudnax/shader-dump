...
float scene(vec3 p) {
...

    {
        ...
        float tooth = sdCone(p * vec3(1., -1., 1.) + vec3(0.2, -0.23, 0.2) + vec3(-0.13, 0.0, 0.09),
                             vec2(0.2, 0.1), 0.05);
        float exterrior =
        ...
        umuer += noise(umuer.yzw * 100) * 0.01 * pow(umuer.y, 3);
...

    {
        ...
        float ang = atan(pi.x, pi.z);
        vec3 dripping =
            sdCappedCone(pi, vec3(0.), vec3(0., 0.13, 0.), 0.25, 0.20);
        /* dripping = torus(pi, .1, .48); */
        dripping.z += -0.113;
        float waveFactor = (d > 1. ? 1. : .5);
        float wave = sin(ang * 12. * waveFactor) * .002 +
                     sin(ang * 34. * waveFactor + .7) * 0.017 +
                     sin(ang * 42. * waveFactor + 1.7) * 0.015;

        vec3 icingWave = vec3(0, 0, pi.y - (wave + .03));
         /* icingWave = vec3(0, 0, pi.y  ); */
        vec3 icing = carve(dripping, icingWave, .2);

		res = min(res, icing.z);
		/* res = icing.z; */

		/* res = min(res, dripping.z); */
    }
